...
export const useDataStore = defineStore('data', () => {
  ...
  const allSkillDatas = computed(() => Array.from(skillDataMap.values()).sort((a, b) => a.sort - b.sort));
  ...
  function initData(data: DataJson) {
    ...
    for (const gatheringAreaJson of data.gatheringAreas) {
      const gatheringArea = markRaw(GatheringArea.fromJson(gatheringAreaJson, itemMap, skillDataMap));
